$(document).ready(function () {
    ...
    $("#table_list").bootstrapTable({
        ...
        minimumCountColumns: 2,    //最少允许的列数ss
        //表示服务端请求
        ...
        columns: [
            ...
            {title: "查询结果",field: "queryResults"},
            {title: "预期结果",field: "overdueResults"},
            {title: "创建日期",field: "modify",
                formatter:function (value, row, index) {
                    return changeDateFormat(value)
            }},
            {title: "执行时间",field: "executionTime",
                    formatter:function (value, row, index) {
                        return changeDateFormat(value)
            }},
            {title: "状态",sortable: true,field: "status",
                formatter: function (value, row, index) {
                    if (value == '1')
                        ...
                        return '<span class="label label-info">审核未通过</span>';
                    else
...
function doSubmission() {
    ...
    if(selectNodes.length == 1){
        layer.confirm('确定执行吗?', {icon: 3, title:'提示'}, function(){
            $.ajax({
                type: "POST",
                dataType: "json",
                url: '/submission/show?id='+selectNodes[0].id,
                success: function(ret){
                    if(ret.success){
                        layer.msg("执行成功！", {time: 1500},null);
                        $('#table_list').bootstrapTable("refresh");
                    }else{
                        layer.msg("执行失败："+ret.message, {time: 1500,icon:2});
                    }
                }
            });
        });
...
function editSubmission(){
    ...
    if(selectNodes.length == 1){
        layer.open({
            ...
            shade: 0.5,
            area: ['85%', '80%'],
            content: '/submission/edit?id='+selectNodes[0].id,
            end: function(){
                $('#table_list').bootstrapTable("refresh");
            }
        });
    ...
    if(selectNodes.length == 1){
        layer.confirm('确定删除吗?', {icon: 3, title:'提示'}, function(){
            $.ajax({
                type: "POST",
                dataType: "json",
                url: '/submission/delUser?id='+selectNodes[0].id,
                success: function(ret){
                    if(ret.success){
